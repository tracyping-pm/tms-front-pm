...
export default (props: {
      ...
      fieldParse?.forEach((item) => {
        if (formItemData?.fieldType === FieldTypeEnum.CUSTOMER_CHARGE_GROUP) {
          let toStr = JSON.stringify({
            ...
            order: item?.order || item?.fieldOrder,
  ...
  const DynamicFormItem = useMemo(() => {
    ...
    let defaultSelectedValue: any; // 选项
    ...
    const fieldValue =
      formItemData.fieldValue && JSON.parse(formItemData.fieldValue!);
    ...
    if (
      ...
      defaultSelectedValue = valueOptions?.find?.(
    ...
    switch (type) {
            ...
            rules={[
              {
                ...
                message: `Please Enter ${formItemData.fieldName}`,
              },
              {
                whitespace: true,
              ...
              {
                max: MAX_LENGTH.NOTE,
                message: `${formItemData.fieldName} cannot exceed ${MAX_LENGTH.NOTE} characters`,
              },
            ]}
          >
            <CustomFormInput placeholder="Please Enter" readOnly={readOnly} />
          </Form.Item>
        );
          ...
          <ProFormTextArea
            ...
            rules={[
              ...
              },
              {
                ...
                message: `${formItemData.fieldName} cannot exceed ${MAX_LENGTH.MAX_5000} characters`,
              },
            ]}
          />
        );
      ...
      case FieldTypeEnum.TIME_PICKER:
        return (
          <ProFormDateTimePicker
            name={[formItemData.order, 'fieldValue']}
            label={
      ...
      case FieldTypeEnum.CUSTOMER_CHARGE:
      case FieldTypeEnum.VENDOR_CHARGE:
        return (
          <Row>
            <Col span={6}>
              <CustomTooltip
                title={formItemData.fieldName ?? '-'}
                placement="topRight"
              >
                <div
                  className={cls(
                ...
                >
                  ...
                  <div className={styles.customLabelChargeSymbol}>:</div>
                </div>
              </CustomTooltip>
            </Col>
            <Col span={18} style={{ display: 'flex' }}>
              {!readOnly ? (
                <>
                  <ProFormSelect
                    name={[formItemData.order, 'symbol']}
                    label={null}
                    initialValue={'None'}
                    valueEnum={{
                      None: 'None',
                  ...
                  />
                  <ProFormDependency name={[formItemData.order, 'symbol']}>
                    {(data) => {
                      return (
                        <ProFormDigit
                          name={[formItemData.order, 'fieldValue']}
                ...
                <>
                  {`${fieldValue?.symbol === 'None' ? '' : fieldValue?.symbol} ${CountryCurrencyEnumText[countryId as any]} ${fieldValue?.fieldValue}`}
                </>
              ) : (
                '-'
              )}
              ...
              return (
                <div key={c}>
                  ...
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: option.title !== 'None' ? true : false,
  ...
  }, [formItemData, noRequestFiles, readOnly, customerList, vendorList]);
