import { refundEditOcStatus } from '@/api/claim';
import { IRefundDetail } from '@/api/types/claims';
import { EnumClaimOcStatus, ocStatusRefundOptions } from '@/enums/claim';
...
const OcStatusModal: FC<IProps> = ({
  ...
  const onSubmit = async () => {
    ...
    setSubmitting(true);
    const res = await refundEditOcStatus(payload).finally(() => {
      setSubmitting(false);
    });
    if (res.code === 200) {
