import { vendorAnalysisByCustomerVendorList } from '@/api/statistics';
import {
...
interface ISubTableProps {
  ...
  cache?: Map<number, IVendorAnalysisByCustomerVendorItem[]>;
}
...
const SubTable: FC<ISubTableProps> = memo(
  ({
    ...
    const fetchDataSource = async () => {
      ...
      const payload: IVendorAnalysisByCustomerPayload = {
        yearMonth,
        customerId,
        waybillTimeType,
        comparisonPeriod,
      ...
      setLoading(true);
      const res = await vendorAnalysisByCustomerVendorList(payload).finally(
        () => {
          setLoading(false);
        },
      );
    ...
    const columns: ColumnsType<IVendorAnalysisByCustomerVendorItem> =
      useMemo(() => {
        const waybillChangeCompareColumns =
          generateCompareColumns<IVendorAnalysisByCustomerVendorItem>({
        ...
        return [
          ...
          {
            title: (
              <TooltipTitle tips="The time when the vendorr's owned truck completed its first waybill (Delivered/Abnormal) for this customer.">
                First Delivery date
              </TooltipTitle>
            ),
...

          {
            ...
            render: (_, record) => {
              ...
              const _customerName = customerName
                ? `&customerName=${encodeURIComponent(customerName as string)}`
                : '';
              ...
              return (
                <Flex gap={12}>
                  <Link
                    to={`${PATHS.VENDOR_STATISTIC_BY_CUSTOMER_TREND_DETAIL}?year=${year}&customerId=${customerId}${_customerName}&vendorId=${record.vendorId}${vendorName}&waybillTimeType=${waybillTimeType}`}
                    target="_blank"
                  >
                    Trend
                  </Link>
                  <Link
                    to={`${PATHS.VENDOR_STATISTIC_VENDOR_COMPARISON}?yearMonth=${yearMonth}&customerId=${customerId}&vendorId=${record.vendorId}${vendorName}&waybillTimeType=${waybillTimeType}`}
                    target="_blank"
                  >
                    Compare
                  </Link>
      ...
      }, [yearMonth, customerId, customerName, sortField, sortOrder]);
    ...
    return loading ? (
      ...
      <Table<IVendorAnalysisByCustomerVendorItem>
