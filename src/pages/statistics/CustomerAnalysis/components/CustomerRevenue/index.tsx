import { customerAnalysisRevenueStatic } from '@/api/statistics';
import {
...
export default function Revenue() {
  ...
  const [sourceData, setSourceData] = useState<ICustomerRevenueRecord[]>([]);
  ...
  const [topValue, setTopValue] = useState<number>(TOP);
  ...
  const formatBarData = () => {
    const list: ICustomerRevenueRecord[] = sourceData;
    if (!list.length) {
      return;
    }
    ...
    const _list = list.map((item: ICustomerRevenueRecord) => {
      const o = {
  ...
  const fetchDataSource = async (date: Dayjs) => {
    ...
    const res = await customerAnalysisRevenueStatic(payload).finally(() =>
      setLoading(false),
    );
  ...
  return (
    <CardView
      title={
        <TooltipTitle tips="Customer Revenue:Displays customers  those whose status was changed to In Service on or before the selected month">
          {'Customer Revenue'}
        </TooltipTitle>
      }
    >
