import { customerAnalysisActiveProjectStatic } from '@/api/statistics';
import {
...
export default function ProjectStatistic() {
  ...
  const [dataSource, setDataSource] = useState<IActiveProjectStaticRecord[]>(
    [],
  );
  ...
  const fetchDataSource = async (date: Dayjs) => {
    ...
    const res = await customerAnalysisActiveProjectStatic(payload).finally(() =>
      setLoading(false),
    );
    if (res.code === 200) {
      const data = res.data ?? [];
  ...
  return (
    <CardView title={'Project Statistic'}>
      <Row gutter={12}>
        <Col span={12}>
