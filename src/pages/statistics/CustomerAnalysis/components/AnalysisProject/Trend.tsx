import { projectAnalysisAnnualRevenueStatistics } from '@/api/statistics';
import {
...
const Trend: FC = ({}) => {
  ...
  const fetchDataSource = async () => {
    const payload: IProjectAnalysisParams = {
    ...
    setLoading(true);
    const res = await projectAnalysisAnnualRevenueStatistics(payload).finally(
      () => {
        setLoading(false);
      },
    );
  ...
  const columns: TableColumnsType<IProjectAnalysisAnnualRevenueStatisticsRecord> =
    [
      {
        title: 'Month',
        dataIndex: 'month',
  ...
  const getSortedData = (
    data: IProjectAnalysisAnnualRevenueStatisticsRecord[],
    sorterObj: any,
  ) => {
    ...
    const field =
      sorter.field as keyof IProjectAnalysisAnnualRevenueStatisticsRecord;
  ...
  return (
    <Flex gap={12} vertical>
      <CardView
        title={`${dayjs(yearMonth).format('YYYY')} Project Analysis Detail: ${projectName}`}
      >
        <Table<IProjectAnalysisAnnualRevenueStatisticsRecord>
          rowKey="month"
          size="small"
          loading={loading}
          columns={columns}
