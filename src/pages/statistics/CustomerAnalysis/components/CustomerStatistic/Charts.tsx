...
const Charts = ({
  ...
  useEffect(() => {
    const data: any = {
      mouthDate: [],
      existingActiveCustomer: [],
      existingReactiveCustomer: [],
      lost: [],
      newCustomer: [],
    };
    dataSource.forEach((item: any) => {
      data.mouthDate.push(item.mouthDate);
      data.existingActiveCustomer.push(item.existingActiveCustomer);
      data.existingReactiveCustomer.push(item.existingReactiveCustomer);
      data.lost.push(item.lostCustomer);
      data.newCustomer.push(item.newCustomer);
    });
    ...
    const processStackData = () => {
      ...
      mouthDate.forEach((_: string, index: number) => {
        const valActive = existingActiveCustomer[index] || 0;
        const valReactive = existingReactiveCustomer[index] || 0;

        const valNew = newCustomer[index] || 0;
    ...
    const option = {
      ...
      series: [
        ...
        {
          color: '#FA8C16',
          name: 'Existing Reactive Customer',
          type: 'bar',
          barMaxWidth: BAR_WIDTH,
          stack: 'total',
