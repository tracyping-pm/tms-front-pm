...
import { dataProcessingLogs, waybillRegenerateSubtask } from '@/api/tool';
...
const WaybillSubtaskModify: React.FC = () => {
  ...
  const onFinishHandle = async (values: any) => {
    ...
    const res = await waybillRegenerateSubtask(ids);
    setLoading(false);
    if (res.code === 200) {
      message.success('Waybill Regenerate Subtask successfully');
  ...
  const fetchLogList = useCallback(async () => {
    ...
    const res = await dataProcessingLogs('WaybillRegenerateSubtask').finally(
      () => {
  ...
  return (
    <div className={style.content}>
      <div className={style.title}>
        <CommonTitle
          title="Regenerate Waybill Subtask"
          extra={
            <CustomStatusButton
              noStyle
...
export default WaybillSubtaskModify;
