...
import {
  vendorContractAdd,
  vendorContractTrackingExpireCount,
  vendorContractTrackingList,
} from '@/api/vendor';
...
const ContractTracking: FC = () => {
  ...
  const [originData, setOriginData] =
    useState<PaginationResponse<IVendorContractTrackingItem>>(
      DEFAULT_PAGINATION,
    );
  const [noteModalOpen, setNoteModalOpen] = useState(false);
  const [activeRecord, setActiveRecord] =
    useState<IVendorContractTrackingItem>();
  ...
  const getFormValue = () => {
    ...
    const payload = {
      ...
      vendorId: values?.vendorNameObj?.id,
      projectActivityStatus: values?.projectActivityStatus,
      customerId: values?.customerNameObj?.id,
      vendorAccreditationStatus: values?.vendorAccreditationStatus,
      vendorActivityStatus: values?.vendorActivityStatus,
      operatorType: values?.operatorType,
      operatorDays: values?.operatorDays,
      picUserId: values?.viewMineOnly ? userId : values?.aliasNameObj?.id,
    };
    return payload;
  };
  ...
  const getExpireCount = async () => {
    ...
    const res = await vendorContractTrackingExpireCount().finally(() => {
  ...
  const getTableDataSource = async (
    payload?: IVendorContractTrackingListParams,
  ) => {
    setTableLoading(true);
    const res = await vendorContractTrackingList(payload ?? {}).finally(() => {
  ...
  const reloadTableData = (options?: {
    ...
    let payload: IVendorContractTrackingListParams = {};
  ...
  const onAddNote = (record: IVendorContractTrackingItem) => {
    setActiveRecord(record);
    setNoteModalOpen(true);
  };

  const onDeleteNote = async (record: IVendorContractTrackingItem) => {
  ...
  const onUpdateContract = (record: IVendorContractTrackingItem) => {
    setActiveRecord(record);
  ...
  const onUpdateContractConfirm = async (
    ...
    const res = await vendorContractAdd(formData).finally(() => {
  ...
  const columns: TableProps<IVendorContractTrackingItem>['columns'] = [
    {
      title: 'No.',
      dataIndex: 'No.',
      key: 'No.',
    ...
    },
    {
      ...
      width: 170,
      render: (_value, record) => (
        <CustomTooltip title={record.picName}>
    ...
    },
    {
      ...
      dataIndex: 'vendorAccreditationStatus',
      key: 'vendorAccreditationStatus',
      width: 220,
      render: (_, record) => {
        const status: VendorStatusEnum = record.vendorAccreditationStatus;
        const Content = (
          <Badge
    ...
    },
    {
      title: (
        <Space>
          ...
          <Tooltip
            title={`A vendor is classified as "Active" if they have non-canceled waybills under the project within the last 30 days; otherwise, they are classified as "Inactive."`}
          >
            <InfoCircleOutlined />
          </Tooltip>
        </Space>
      ),
      dataIndex: 'vendorActivityStatus',
      key: 'vendorActivityStatus',
      width: 200,
      render: (_, record) => {
        const color = record.vendorActivityStatus ? 'green' : 'red';
        const text = record.vendorActivityStatus ? 'Active' : 'Inactive';
        const Content = <Badge color={color} text={text} />;
        return <Popover title={Content}>{Content}</Popover>;
      },
    },
    {
      title: 'Vendor Contract Period',
      dataIndex: 'vendorContractPeriod',
      key: 'vendorContractPeriod',
      width: 200,
      render: (_value, record) => (
        <Flex vertical>
          <span>Start: {record.vendorContractPeriod?.startDate}</span>
          <span>End: {record.vendorContractPeriod?.endDate}</span>
        </Flex>
      ),
    },
    {
    ...
    {
      title: 'Note',
      dataIndex: 'vendorNote',
      key: 'vendorNote',
      width: 240,
      render: (_value, record) => {
        const { vendorNote } = record;
        if (!vendorNote) return '-';
        const { note, createdName, createdAt } = vendorNote;
        return (
    ...
    },
    {
      ...
      render: (_value, record) => (
        <Space split={<Divider type="vertical" />}>
          ...
          {access[PermissionEnum.VENDOR_CONTRACT_TRACKING_UPDATE] && (
            <Button
              color="primary"
              variant="link"
              style={{ padding: 0 }}
  ...
  return (
    <>
      <Flex gap={12} vertical>
        ...
        <Form form={form} name="vendor-contract-tracking-form">
          <Flex gap={10} align="center" wrap>
            ...
            <section>
              <Form.Item name="vendorNameObj" noStyle>
                <FuzzySelector
                  fieldProps={{
            ...
            <section>
              <Form.Item name="vendorAccreditationStatus" noStyle>
                <Select
                  placeholder="Vendor Accreditation Status"
                  options={[
                    {
                      label:
                        VendorStatusEnumText[VendorStatusEnum.UNACCREDITED],
                      value: VendorStatusEnum.UNACCREDITED,
                    },
                    {
                      label: VendorStatusEnumText[VendorStatusEnum.ACCREDITED],
                      value: VendorStatusEnum.ACCREDITED,
                    },
                    {
                      label: VendorStatusEnumText[VendorStatusEnum.TERMINATED],
                      value: VendorStatusEnum.TERMINATED,
                    },
                  ]}
                  style={{ width: `${DEFAULT_WIDTH}px` }}
                  allowClear
            ...
            <section>
              <Form.Item name="vendorActivityStatus" noStyle>
                <Select
                  placeholder="Vendor Activity Status"
                  options={[
                    { label: 'Active', value: true },
                    { label: 'Inactive', value: false },
                  ]}
            ...
            <section>
              <Form.Item name="aliasNameObj" noStyle>
                <FuzzySelector
            ...
            <section>
              <Form.Item name="viewMineOnly" valuePropName="checked" noStyle>
                <Checkbox>View Mine Only</Checkbox>
              </Form.Item>
            </section>
        ...
        <section>
          <Table<IVendorContractTrackingItem>
            scroll={{ x: 3000 }}
            columns={columns}
      ...
      {activeRecord ? (
        <UpdateContractModal
          contractId={activeRecord.contractId}
          contractType={ContractTypeEnum.VENDOR}
          projectInfo={{
            id: activeRecord.projectId,
            name: activeRecord.projectName,
          }}
          contractSignerInfo={{
            id: activeRecord.vendorId,
