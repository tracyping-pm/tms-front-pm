...
import { IVendorDetailContractListPayload } from '@/api/types/vendor';
import { vendorContractList } from '@/api/vendor';
...
const VendorDetailContractList: React.FC = () => {
  const { message } = App.useApp();
  const access = useAccess();
  const { id } = useParams();
  const vendorId = id ? +id : undefined;
  ...
  const getDataSource = useCallback(
    async (BE_NEED: IVendorDetailContractListPayload) => {
      setLoading(true);
      const res = await vendorContractList(BE_NEED);
      setLoading(false);
  ...
  const onSubmit = useCallback(async (params: any) => {
    ...
    const BE_NEED: IVendorDetailContractListPayload = {
      pageNum,
      pageSize,
      vendorId,
    };
  ...
  //           };
  //           setVoidLoading(true);
  //           const voided = await contractVoid(payload);
  //           setVoidLoading(false);
  //           if (voided.code === 200) {
  ...
  const columns: ProColumns[] = [
    ...
    },
    {
      ...
      width: 284,
      hideInTable: !(
        access[PermissionEnum.VENDOR_DETAIL_CONTRACTS_VIEW] ||
        access[PermissionEnum.VENDOR_DETAIL_CONTRACTS_DOWNLOAD] ||
        access[PermissionEnum.VENDOR_DETAIL_CONTRACTS_HISTORY]
      ),
      render: (_, record) => {
        const renderContent = () => {
        ...
        const operationList = [
          ...
          access[PermissionEnum.VENDOR_DETAIL_CONTRACTS_HISTORY]
            ...
            : null,
          // access[PermissionEnum.VENDOR_DETAIL_CONTRACTS_TERMINATE] &&
...
export default VendorDetailContractList;
