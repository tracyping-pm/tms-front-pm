import {
  createTruckAccreditationVersion,
  deleteTruckCategory,
  updateTruckAccreditation,
} from '@/api/truck';
...
export default function ListItem({
  ...
  const handleDeleteItem = async () => {
    modal.confirm({
      ...
      cancelText: 'Cancel',
      onOk: async () => {
        const res = await deleteTruckCategory({
          id: truckId,
          fileCategory,
        });
        if (res.code === 200) {
  ...
  const onEditCategoryConfirm = async (v: IEditCategoryConfirm) => {
    ...
    let apiFuc;
    if (v.version) {
      apiFuc = updateTruckAccreditation({
        id: v.id,
      ...
      apiFuc = createTruckAccreditationVersion(v);
    }
    ...
    <>
      <div className={styles.item}>
        <div className={styles.item_title}>
          ...
          {!defaultCategory &&
            access[PermissionEnum.TRUCK_DETAIL_ACCREDITATION_EDIT] && (
              <span
