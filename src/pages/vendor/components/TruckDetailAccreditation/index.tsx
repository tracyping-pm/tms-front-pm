...
import { addTruckCategory, truckAccreditationList } from '@/api/truck';
...
export default memo(function TruckDetailAccreditation(props: {
  showModal: boolean;
  ...
  const fetchData = async () => {
    setLoading(true);
    const res = await truckAccreditationList({ id: Number(truckId) });
    setLoading(false);
    if (res.code === 200) {
      const font =
  ...
  const addModalFinish = async (params: any) => {
    setLoading(true);
    const res = await addTruckCategory({
      id: Number(truckId),
  ...
  useEffect(() => {
    const unsubscribe = subscribe('EVENT_ACCREDITATION_TRUCK_RELOAD', () => {
      fetchData();
    });
