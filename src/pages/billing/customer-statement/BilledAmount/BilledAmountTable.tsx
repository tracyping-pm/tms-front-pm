...
export default function BilledAmountTable({
  ...
  const countAmount = (record: any) => {
    ...
    if (obj) {
      ...
      ].includes(record.basicAmountStatus)
        ? 0
        : (obj?.basicAmount ?? 0);
      ...
      amount = +numberSum(
        [basicAmount, additionalCharge, exceptionFee, miscellaneousCharge],
        2,
      );
    }
  ...
  const columns: ProColumns[] = [
    ...
    {
      title: `Actual Basic Amount Receivable(${countryCurrency})`,
      ...
      render: (_, record) => {
        const { basicAmount, basicAmountChange, basicAmountStatus } = record;
        const text =
          !basicAmount && basicAmount !== 0 ? '' : formatAmount(basicAmount);
        ...
        return isEdit &&
          ...
          <div className={styles.editAmount}>
            <InputNumber
              ...
              controls={false}
              onBlur={(event) => {
                saveAmountChange(record, event.target.value, 'basicAmount');
              }}
            />
          </div>
        ) : (
          <div>
            <CustomTooltip title={text} placement="top">
              <span style={{ color: basicAmountChange ? '#1890ff' : '' }}>
                {text}
              </span>
            </CustomTooltip>
          </div>
    ...
    {
      title: `Actual Customer Additional Charge(${countryCurrency})`,
      ...
      render: (_, record) => {
        ...
        return isEdit &&
          ...
          <div>
            <CustomTooltip title={text} placement="top">
              <span
                // className={cls(styles[additionalChargeStatus])}
                style={{ color: additionalChargeChange ? '#1890ff' : '' }}
              >
                {text}
              </span>
            </CustomTooltip>
    ...
    {
      title: `Actual Customer Exception Fee(${countryCurrency})`,
