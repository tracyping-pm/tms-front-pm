...
import { customerTaxMark } from '@/api/customer';
...
const Step1: FC = () => {
  ...
  const customerNameValue = Form.useWatch('customerName', form);
  ...
  const [projectSelectedRowKeys, setProjectSelectedRowKeys] = useState<
    React.Key[]
  >([]);
  const [customerTaxMarkLoading, setCustomerTaxMarkLoading] =
    useState<boolean>(false);
  const [customerTaxMarkValue, setCustomerTaxMarkValue] = useState<string>('');
  ...
  const fetchProject = async () => {
    ...
    const entityId = values?.customerName?.value;
    if (entityId) {
      setProjectLoading(true);
      const res = await statementQueryProject({
        type: StatementTypeEnum.CUSTOMER,
        entityId,
      }).finally(() => {
  ...
  const fetchCustomerTaxMark = async () => {
    const values = form.getFieldsValue();
    const entityId = values?.customerName?.value;
    if (entityId) {
      setCustomerTaxMarkLoading(true);
      const res = await customerTaxMark({
        id: entityId,
      }).finally(() => {
        setCustomerTaxMarkLoading(false);
      });

      if (res.code === 200) {
        setCustomerTaxMarkValue(res.data ?? '');
  ...
  const handleGenerate = async () => {
    ...
    const payload: IStatementAddReq = {
      type: StatementTypeEnum.CUSTOMER,
      entityId: values.customerName.value,
      ...
      // settlementTimeType: values.settlementTimeType,
    ...
    if (res.code === 200) {
      ...
      history.replace(`${PATHS.BILLING_CUSTOMER_STATEMENT_DETAIL}/${detailId}`);
    }
  };
  ...
  const handleNext = async () => {
    ...
    const step1Data = {
      type: StatementTypeEnum.CUSTOMER,
      entityId: formValues.customerName.value,
  ...
  useEffect(() => {
    ...
    if (isBasedOnWaybillValue) {
      ...
      if (customerNameValue) {
        fetchProject();
        fetchCustomerTaxMark();
      } else {
        setProjectData([]);
        setCustomerTaxMarkValue('');
  ...
  }, [isBasedOnWaybillValue, customerNameValue]);
    ...
    <>
      <div className={styles.step1}>
        <section className="basic-setting">
          ...
          <Form
            form={form}
            name="customer-statement-basic-settings"
            // layout="vertical"
          >
            ...
            {isBasedOnWaybillValue && (
              <>
                <div className="case-item">
                  ...
                  >
                    <CommonCheckboxCombo
                      plainOptions={CustomerSettledItemListOptions}
                    />
                  </Form.Item>
                </div>
                ...
                <div className="case-item">
                  <div className="inline-form">
                    <div className="inline-form-item">
                      <Form.Item
                        label={'Customer Tax Mark'}
                        layout="horizontal"
                      >
                        <span>
                          {customerTaxMarkLoading ? (
                            <Spin spinning={true} size="small" />
                          ) : (
                            customerTaxMarkValue || '-'
                          )}
                        </span>
                      </Form.Item>
                    </div>
