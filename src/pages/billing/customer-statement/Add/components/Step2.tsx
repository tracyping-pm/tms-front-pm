...
import DetailWaybillListCard from '@/pages/billing/components/customer/DetailWaybillListCard';
...
const Step2: FC = () => {
  ...
  const { getStepData, doPrev } = useModel('bill.addCustomerStatement');
  ...
  const handleGenerate = async () => {
    ...
    if (res.code === 716) {
      ...
      if (res.code === 200) {
        ...
        history.replace(
          `${PATHS.BILLING_CUSTOMER_STATEMENT_DETAIL}/${detailId}`,
        );
      } else {
