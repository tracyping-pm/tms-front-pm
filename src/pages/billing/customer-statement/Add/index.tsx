...
const AddStatement = () => {
  ...
  const { state, reset } = useModel('bill.addCustomerStatement');
  console.log({ state });

  const goList = useCallback(() => {
    modal.confirm({
      title: 'AR Statement List',
      content: 'Confirm return to the AR Statement List',
      okText: 'Confirm',
      width: 530,
      maskClosable: false,
      closable: true,
      ...
      onOk() {
        reset();
        history.replace(PATHS.BILLING_CUSTOMER_STATEMENT);
      },
      onCancel() {
        // do nothing
      },
