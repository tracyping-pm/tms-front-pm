...
import {
  getCustomerStatementList,
  getCustomerStatementListExport,
} from '@/api/billing';
import { ICustomerStatementParams } from '@/api/types/billing';
...
import {
  ...
  CustomerSettledItemListOptions,
...
const CustomerStatementList: React.FC = () => {
  ...
  const getDataSource = async (BE_NEED: IBE_NEED) => {
    setLoading(true);
    const res = await getCustomerStatementList(
      BE_NEED as ICustomerStatementParams,
    );
    setLoading(false);
    if (res.code === 200) {
  ...
  const onSubmit = async (params: any) => {
    ...
    if (values.outstandingAmount) {
      lodash.set(
        FE_NEED,
        'outstandingAmountStart',
        values.outstandingAmount[0],
      );
      lodash.set(FE_NEED, 'outstandingAmountEnd', values.outstandingAmount[1]);
      ...
      lodash.set(BE_NEED, 'outstandingAmountEnd', values.outstandingAmount[1]);
    }
  ...
  const fillTableForm = (FE_NEED: IFE_NEED) => {
    formRef.current?.setFieldsValue({
      ...
      outstandingAmount: [
        FE_NEED.outstandingAmountStart,
        FE_NEED.outstandingAmountEnd,
      ],
    });
  ...
  const onReset = () => {
    ...
    setWaybillNumberValue(undefined);
  };
  ...
  const doDownload = async () => {
    ...
    const res = await getCustomerStatementListExport(payload).finally(() => {
  ...
  const columns: ProColumns[] = [
      ...
      fieldProps: {
        ...
        onSearch: (keywords: string) =>
          statementNumberSearch(keywords, {
            ...
            uniqueLogicParams: { type: 'Customer' },
          }),
        value: statementNumberValue,
      },
      render: (_, record) => {
    ...
    },
    {
      ...
      fieldProps: {
        ...
        options: CustomerSettledItemListOptions,
      },
      render: (_, record) => {
        const { settledItemList } = record;
        const content = settledItemList
          ?.map(
            (item: CustomerSettledItemEnum) =>
              CustomerSettledItemEnumText[item],
          )
          .join(', ');
        return content ?? '-';
      },
    ...
    },
    {
      ...
      order: 5,
      valueType: 'select',
      valueEnum: CustomerStatementStatusEnumText,
      formItemProps: {
        label: null,
        style: {
          width: `${DEFAULT_WIDTH}px`,
      ...
      render: (_, record) => {
        const status: CustomerStatementStatusEnum = record.statementStatus;
        const Content = (
          <Badge
            color={CustomerStatementStatusEnumIconColor[status]}
            text={CustomerStatementStatusEnumText[status]}
          />
        );
    ...
    {
      title: `Total Amount Receivable (${CountryCurrencyEnumText[countryId as number]})`,
    ...
    {
      title: `Amount Received (${CountryCurrencyEnumText[countryId as number]})`,
    ...
    {
      title: `Outstanding Amount (${CountryCurrencyEnumText[countryId as number]})`,
      ...
      fieldProps: {
        placeholder: ['Outstanding Amount ', 'Outstanding Amount '],
      },
    ...
    },
    {
      ...
      key: 'option',
      fixed: 'right',
      hideInTable: !access[PermissionEnum.CUSTOMER_STATEMENT_DETAIL],
      width: 100,
      render: (_, record) => {
        return (
          <div
          ...
          >
            <Access
              key="detail"
              accessible={access[PermissionEnum.CUSTOMER_STATEMENT_DETAIL]}
            >
              <Button
                icon={<BarsOutlined />}
                type="link"
                onClick={() => {
                  saveScrollTop();
                  history.push(
                    `${PATHS.BILLING_CUSTOMER_STATEMENT_DETAIL}/${record.id}`,
                  );
                }}
              >
                Detail
  ...
  const toolBarRender = () => [
    <Access
      key="create"
      accessible={access[PermissionEnum.CUSTOMER_STATEMENT_ADD]}
    >
      <Button
        type="primary"
        onClick={() => {
          history.replace(PATHS.BILLING_CUSTOMER_STATEMENT_ADD);
        }}
      >
        Add Statement
      </Button>
    ...
    <Access
      key="create"
      accessible={access[PermissionEnum.CUSTOMER_STATEMENT_EXPORT]}
    >
