...
import AddClaimTicketModal from './AddClaimTicketModal';
import styles from './index.less';

const ClaimTicketList = ({
  statementType,
  statementStatus,
}: {
  ...
  const getDataSource = async (params: {
    ...
    let payload: {
      ...
      claimTicketType: EnumClaimTicketType.CLAIM,
    };
    if (statementType === 'AR') {
  ...
  const onRemoveTicket = async (claimId: number) => {
  ...
  const columns: ProColumns[] = [
          ...
          <CustomTooltip title={record.ticketNumber}>
            {access[PermissionEnum.CLAIM_TICKET_DETAIL] ? (
              <Button
                ...
                style={{ padding: 0 }}
                onClick={() => {
                  // todo: open new tab to view detail
                  openNewTag(
    ...
    },
    {
      ...
      ellipsis: true,
      render: (_, record) => {
        const status: StatementClaimTicketStatusEnum = record.ticketStatus;
        const Content = (
          <Badge
