...
export default memo(function EnterReceiptModal({
  materialList = [],
  onCancel,
  onRefresh,
}: {
  ...
  const submit = async (params: any) => {
    ...
    const payload: IStatementReceiptOrPaymentCreateParams = {
      ...
      receiptTime: dayjs(params.receiptTime).format('YYYY-MM-DD HH:mm:ss'),
      receiptAmount:
        params.receiptSymbol === 'POSITIVE'
          ? params.receiptAmount
          : params.receiptAmount * -1,
    ...
    if (res.code === 200) {
      message.success('Add Receipt Successfully');
    ...
    <>
      ...
      >
        <div style={{ fontSize: 14, lineHeight: '22px' }}>
          Please enter the receiving amount and credentials
        </div>
        <Form
          name="Enter-Receipt"
          form={form}
        ...
        >
          <Form.Item
            ...
            style={{ fontSize: '14px' }}
            label="Receipt Time"
            rules={[{ required: true, message: 'Please select Receipt Time' }]}
          >
            <DatePicker
              showTime
          ...
          >
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <Form.Item
                ...
                rules={[
                  { required: true, message: 'Please enter Receipt Amount' },
                  {
                    validator: (rule, value) => {
                      if (Number(value) > 99999999.99) {
                        return Promise.reject(
                          'Receipt Amount must be less 99999999.99',
                        );
                      } else {
              ...
              >
                <InputNumber
                  ...
                  style={{ width: '100%' }}
                  placeholder="Please enter Receipt Amount"
                  precision={2}
                  controls={false}
                  formatter={(value) =>
                    `${value}`
          ...
          <Form.Item
            ...
            label={
              <>
                ...
                <Tooltip
                  title={
                    'Please upload the payment documents provided by the customer to us (e.g., checks, transfer records, screenshots showing an increase in our bank account balance, etc.).'
                  }
                  placement="top"
                >
