import {
  exportStatementAdditional,
  statementAdditionalList,
} from '@/api/billing';
import {
  IClaimSubtaskItem,
  IStatementClaimItem,
...
const AdditionalChargeList: React.FC = () => {
  ...
  const getDataSource = async (params: {
    ...
    setLoading(true);
    const res = await statementAdditionalList(payload).finally(() => {
      setLoading(false);
    });
    ...
    if (res.code === 200) {
      ...
      const _columns = headers.map((header, index) => {
        ...
        return {
          key: header,
          title: (
            <CustomTooltip title={header} placement="top">
              {header}
            </CustomTooltip>
          ),
  ...
  const onExportAll = async () => {
    ...
    const res = await exportStatementAdditional({
      id: Number(statementId),
    }).finally(() => {
