import {
  ...
  IWaybillReimbursementDataItem,
} from '@/api/types/waybill';
import {
  editWaybillReimbursement,
  getWaybillReimbursementOptions,
} from '@/api/waybill';
...
type IWaybillAdditionalChargeModal = ModalFormProps & {
  ...
  detail: IWaybillReimbursementData;
  refresh: () => void;
  cancel: () => void;
};
enum SYMBOL_ENUM {
...
const WaybillReimbursementModal: FC<IWaybillAdditionalChargeModal> = ({
  width = 1200,
  open,
  isStandardWaybill,
  detail,
  ...
  const [customers, setCustomers] = useState<IWaybillReimbursementDataItem[]>(
    [],
  );
  const [vendors, setVendors] = useState<IWaybillReimbursementDataItem[]>([]);
  ...
  const getItemOptions = async () => {
    if (
      isStandardWaybill
        ? !access[PermissionEnum.STANDARD_WAYBILL_REIMBURSEMENT_OBJECT_EDIT]
        : !access[PermissionEnum.TEMPORARY_WAYBILL_REIMBURSEMENT_OBJECT_EDIT]
    ) {
      return;
    }
    const res = await getWaybillReimbursementOptions();
    if (res.code === 200) {
      let customerItems: any = {};
      let vendorItems: any = {};
      res.data?.customerItemList?.forEach((item) => {
        customerItems[item] = item;
      });
      res.data?.vendorItemList?.forEach((item) => {
  ...
  const handleOk = async (params: any) => {
    const res = await editWaybillReimbursement({
      waybillId: Number(waybillId),
      customerReimbExpenseList: customers.map((c: any) => ({
        item: c.item,
        amount:
      ...
      vendorReimbExpenseList: vendors.map((v: any) => ({
        item: v.item,
        amount:
      ...
      reimbExpenseReceivableStatus: params?.customerStatus,
      reimbExpensePayableStatus: params?.vendorStatus,
      customerCanUpdate: detail.customerCanUpdate,
      vendorCanUpdate: detail.vendorCanUpdate,
    });
    if (res.code === 200) {
      refresh?.();
      message.success('Edit Reimbursement Expense successfully!');
      cancel();
    }
  };
  ...
  useEffect(() => {
    getItemOptions();
    const addLf = detail?.customerReimbExpenseList ?? [];
    const addRt = detail.vendorReimbExpenseList ?? [];
    formRef?.current?.setFieldValue(
      'customerStatus',
      detail?.reimbExpenseReceivableStatus,
    );
              ...
              <div className={styles.additional_left}>
                <div className={styles.additional_title}>
                  ...
                  (isStandardWaybill
                    ? !access[
                        PermissionEnum
                          .STANDARD_WAYBILL_REIMBURSEMENT_STATUS_EDIT
                      ]
                    : !access[
                        PermissionEnum
                          .TEMPORARY_WAYBILL_REIMBURSEMENT_STATUS_EDIT
                      ]) ? (
                    <Badge
                      color={
                        BillingStatusEnumColor[
                      ...
                      text={
                        BillingStatusText[detail.reimbExpenseReceivableStatus]
                      }
                    />
                  ) : (
                    <ProFormSelect
                          ...
                          <ProFormSelect
                            ...
                            disabled={
                              ...
                              (isStandardWaybill
                                ? !access[
                                    PermissionEnum
                                      .STANDARD_WAYBILL_REIMBURSEMENT_OBJECT_EDIT
                                  ]
                                : !access[
                                    PermissionEnum
                                      .TEMPORARY_WAYBILL_REIMBURSEMENT_OBJECT_EDIT
                                  ])
                            }
                            fieldProps={{
                              style: {
              ...
              <div className={styles.additional_right}>
                <div className={styles.additional_title}>
                  ...
                  (isStandardWaybill
                    ...
                    <Badge
                      color={
                        BillingStatusEnumColor[detail.reimbExpensePayableStatus]
                      }
                      text={BillingStatusText[detail.reimbExpensePayableStatus]}
                    />
                  ) : (
                    <ProFormSelect
                      name={'vendorStatus'}
...
export default WaybillReimbursementModal;
