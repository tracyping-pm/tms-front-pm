...
const Step2: FC<IStep2> = ({ waybillId, doPrev, doCancel, onConfirm }) => {
  ...
  const handleConfirm = useCallback(async () => {
    modal.confirm({
      ...
      okText: 'Confirm',
      onOk: async () => {
        const { originList, destinationList, routeCode } = step1DataRef.current;
  ...
  useEffect(() => {
    if (map) {
      const doDirectionsService = (travelMode: google.maps.TravelMode) => {
        ...
        const { originList, destinationList } = step1DataRef.current;
  ...
  useEffect(() => {
    const unsubscribe = subscribe(STEP_EVENTS.STEP1_NEXT_TRIGGER, (payload) => {
      doReset();
      const copiedPayload = cloneDeep(payload);
      step1DataRef.current = copiedPayload;
      setMapVisible(true);
    });
