...
const Item = memo(function ({
  ...
  const url = `/api/customer/business-document/material/add`;
  const dto = {
    id: customerId,
    fileCategory,
    defaultCategory,
  ...
  const handleDeleteItem = async () => {
    modal.confirm({
      ...
      cancelText: 'Cancel',
      onOk: async () => {
        const res = await deleteBusinessType({
          id: customerId,
  ...
  const handleDeleteMaterial = async (fileItem: IAccreditationFileItem) => {
    const res = await deleteBusinessDocMaterial({
      id: customerId,
...
export default memo(function CustomerDetailAccreditation(props: {
  ...
  const fetchData = async () => {
    setLoading(true);
    const res = await getBusinessDocumentList({ id: +pageId! });
    setLoading(false);
    if (res.code === 200) {
  ...
  const addModalFinish = async (values: any) => {
    ...
    setLoading(true);
    const res = await addBusinessType(options);
    setLoading(false);
    if (res.code === 200) {
      ...
      <Spin spinning={loading || imageState.pending}>
        <div className={styles.accreditation}>
          ...
          {/* <div className={styles.add}>
            <Access
              accessible={
                access[PermissionEnum.CUSTOMER_DETAIL_DOCUMENTATIONS_EDIT] &&
                searchParams.get('from') !== CUSTOMER_LEADS_POOL
              }
            >
